import {
  VirdFormNodeAttributes,
  VirdNodeAttributes
} from './vird-node-attributes'
import { VirdNodeTypes } from './vird-node-types'

/** A type to get VirdNode. */
export type ToVirdNode<T extends string | Node | VirdNode> = T extends VirdNode
  ? T
  : T extends VirdNodeTypes['text'] | Text
  ? VirdTextNode
  : T extends VirdNodeTypes['comment'] | Comment
  ? VirdCommentNode
  : T extends VirdNodeTypes['fragment'] | DocumentFragment
  ? VirdFragmentNode
...
// Unique symbol for determining the VirdNode.
